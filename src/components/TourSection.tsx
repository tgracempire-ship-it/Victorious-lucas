...
export const TourSection: React.FC<TourSectionProps> = ({ onOpenBooking }) => {
  ...
  return (
    <section id="tour" className="py-24 bg-slate-50 relative overflow-hidden">
      {/* Background glow */}
      <div className="absolute top-1/3 right-10 w-80 h-80 bg-sky-200/40 rounded-full blur-[100px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            >
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="space-y-1">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {event.featured && (
                      <span className="px-3 py-0.5 rounded-full text-[10px] font-mono font-bold uppercase bg-sky-600 text-white">
                        FEATURED
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-xs text-slate-700 pt-1 font-mono">
                    <span className="flex items-center gap-1 text-slate-900">
                      <MapPin className="w-3.5 h-3.5 text-sky-600" /> {event.venue}, {event.location}
                    </span>
              ...
              <div className="flex items-center gap-3 w-full lg:w-auto justify-end pt-4 lg:pt-0 border-t lg:border-t-0 border-slate-200">
                <button
                  ...
                  className="px-4 py-2.5 rounded-full bg-white border border-slate-200 text-xs font-semibold text-slate-700 hover:text-sky-600 hover:border-sky-500 transition-all flex items-center gap-1.5 cursor-pointer shadow-xs"
                >
                  {calAdded === event.id ? (
                    <>
                ...
                <button
                  ...
                  className="px-6 py-2.5 rounded-full bg-sky-600 text-white font-bold text-xs uppercase tracking-widest hover:bg-sky-700 transition-all shadow-md shadow-sky-600/20 flex items-center gap-1.5 cursor-pointer"
                >
        ...
        <div className="mt-12 p-8 rounded-3xl bg-white border border-slate-200 flex flex-col md:flex-row items-center justify-between gap-6 text-center md:text-left shadow-md">
          ...
          <button
            onClick={onOpenBooking}
            className="px-6 py-3 rounded-full bg-sky-600 text-white font-bold text-xs uppercase tracking-widest hover:bg-sky-700 transition-all shadow-md shadow-sky-600/20 flex items-center gap-2 flex-shrink-0 cursor-pointer"
          >
